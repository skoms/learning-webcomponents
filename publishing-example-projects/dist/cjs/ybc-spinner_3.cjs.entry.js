...
let StockFinder = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.ybcSymbolSelected = index.createEvent(this, "ybcSymbolSelected", 7);
    this.searchResults = [];
    this.loading = false;
    this.onFindStocks = (e) => {
      this.loading = true;
  ...
  render() {
    return [
      index.h("form", { onSubmit: this.onFindStocks }, index.h("input", { id: "stock-symbol", ref: el => (this.stockNameInput = el) }), index.h("button", { type: "submit" }, "Find!")),
      !this.loading
        ? index.h("ul", null, this.searchResults.map(result => index.h("li", { onClick: () => this.onSelectSymbol(result.symbol) }, index.h("strong", null, result.symbol), " - ", result.name)))
        : index.h("ybc-spinner", null)
    ];
  }
};
...
let StockPrice = class {
  ...
  __stencil_render() {
    ...
    return [
      index.h("form", { onSubmit: this.onFetchStockPrice }, index.h("input", { id: "stock-symbol", ref: el => (this.stockInput = el), value: this.stockUserInput, onInput: this.onUserInput }), index.h("button", { type: "submit", disabled: !this.stockInputValid || this.loading }, "Fetch")),
      index.h("div", null, !this.loading
        ? index.h("p", null, dataContent)
        : index.h("ybc-spinner", null)),
    ];
  }
...
exports.ybc_spinner = Spinner;
exports.ybc_stock_finder = StockFinder;
exports.ybc_stock_price = StockPrice;
